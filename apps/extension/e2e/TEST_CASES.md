...
| `bookmark-health.spec.ts` | 书签体检中心重复项筛选、一键清理重复、批量删除 |
| `trash.spec.ts` | 回收站列表、剩余保留期、批量恢复、清空并留下删除墓碑 |
...
### HEALTH-001 未体检也能筛出重复项并一键清理

- 前置：预置 4 条书签，其中 3 条为同一网址（含 tracking 参数变体），不预置任何体检记录。
- 步骤：
  - 打开 `app.html#health`。
  - 切换到「重复书签」筛选。
  - 点击「清理重复项」并确认。
- 断言：
  - 未体检也能列出重复项，保留项与重复项分别带标记。
  - 每组只保留收藏时间最早的一条。
  - 被清理的记录是软删除且 `updatedAt` 被刷新，删除可经 WebDAV 传播。

### HEALTH-002 勾选后批量删除书签
...
### TRASH-001 列出待清除书签并可恢复

- 前置：预置 1 条正常书签和 2 条已删除书签（`deletedAt` 分别为 1 天前、29 天前）。
- 步骤：
  - 打开 `app.html#trash`。
  - 全选后点击恢复。
- 断言：
...
- 断言：
  - 书签本体与正文一并清除。
  - `local:bookmarkTombstones` 留下对应 `{ id, deletedAt }`，供同步传播删除。
