 ...
 * 构建标题与摘要的用户提示词
 */
function buildTitleSummaryPrompt(
  input: AnalyzeBookmarkInput,
  language: AILanguage = "auto",
): string {
  const cleanUrl = input.url
...
function logRequest(
  ...
  logger.info(`[${taskName}] Request:`, {
    system:
      params.system?.slice(0, 200) +
      (params.system && params.system.length > 200 ? "..." : ""),
    prompt:
      params.prompt.slice(0, 500) + (params.prompt.length > 500 ? "..." : ""),
  });
}
...
export function createAIClient(config: AIClientConfig): AIClient {
   ...
   * 单任务模式：一次性生成所有内容
   */
  async function analyzeBookmarkSingle(
    input: AnalyzeBookmarkInput,
  ...
  async function analyzeBookmarkBatch(
    ...
    try {
      // 并行执行三个独立任务
      const [titleSummaryResult, categoryResult, tagsResult] =
        await Promise.all([
          // 任务1：生成标题与摘要
          generateObject({
            model,
            ...
            .catch((error) => {
              ...
              return {
                object: { title: input.title || "未命名书签", summary: "" },
                usage: undefined,
              };
            }),
...
${text}`;
    ...
    async generateObject<T>(options: {
      ...
      logRequest(debug, "generateObject", {
      ...
      try {
        ...
        logResponse(debug, "generateObject", object);

        return object as T;
      } catch (error) {
        logger.error("Object generation failed:", error);
        throw error;
      }
    },
